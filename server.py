...
try:
    ...
    print("警告: ultralytics库未安装，本地推理功能将不可用。")
...
class DetectionData:
    ...
    def __init__(self, data_dict: Dict[str, Any]):
        ...
        # 本地推理相关
        self.local_inference = data_dict.get('local_inference', False)
        self.image_path = data_dict.get('image_path', None)
    ...
    def has_image(self) -> bool:
        return self.image_base64 is not None or self.image_path is not None
    ...
    def decode_image(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.image_base64:
                ...
                # 从文件读取图像
                self.decoded_image = cv2.imread(self.image_path)
    ...
    def create_annotated_image(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.has_detections():
                ...
                # 如果没有检测结果，可以在图像上添加"无缺陷"标记
                h, w = image.shape[:2]
                cv2.putText(self.annotated_image, "No Defects Detected",
    ...
    def save_image(self, save_dir: str, prefix: str = "") -> Optional[str]:
        """保存图像到文件 - 修改：只有有检测结果时才保存"""
        if not save_dir or not self.has_detections():
            return None
    ...
    def save_image_manually(self, save_dir: str) -> Optional[str]:
        """手动保存图像 - 即使没有检测结果也保存，并添加Save_Manually后缀"""
        if not save_dir:
            return None
        ...
        try:
            ...
            # 生成文件名，添加Save_Manually后缀
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
            has_detections = self.has_detections()

            if has_detections:
                # 如果有检测结果，文件名包含检测数量和Save_Manually
                filename = f"detection_{timestamp}_count{len(self.detections)}_Save_Manually.jpg"
            else:
                # 如果没有检测结果，文件名只包含Save_Manually
                filename = f"detection_{timestamp}_Save_Manually.jpg"
    ...
    def save_local_inference_result(self, save_dir: str) -> Dict[str, Any]:
        """保存本地推理结果"""
        try:
            result = {"status": "success", "saved_files": []}

            if not save_dir:
                return {"status": "error", "message": "未指定保存目录"}
            ...
            if self.has_image():
                # 生成文件名
                original_name = os.path.basename(self.image_path) if self.image_path else "unknown"
                name_without_ext = os.path.splitext(original_name)[0]

                if self.has_detections():
                    # 有检测结果
                    image_filename = f"{name_without_ext}_detected_{timestamp}.jpg"
                    result_filename = f"{name_without_ext}_results_{timestamp}.json"
                else:
                    # 无检测结果
                    image_filename = f"{name_without_ext}_no_defects_{timestamp}.jpg"
                    result_filename = f"{name_without_ext}_no_defects_{timestamp}.json"
                ...
                with open(result_path, 'w', encoding='utf-8') as f:
                    json.dump({
                        ...
                        "inference_time": self.timestamp,
                        "class_distribution": self.get_class_distribution()
                    }, f, indent=2, ensure_ascii=False)
                result["saved_files"].append(result_path)

                result["image_saved"] = True
                result["has_detections"] = self.has_detections()
            else:
...
class FileManager:
    ...
    def __init__(self, base_dir: str = ""):
        ...
        self.local_inference_dir = os.path.join(base_dir, "local_inference") if base_dir else ""
    ...
    def _create_dirs(self):
        ...
        try:
            ...
            os.makedirs(self.local_inference_dir, exist_ok=True)
        except Exception as e:
            print(f"创建目录失败: {e}")
    ...
    def set_base_dir(self, base_dir: str):
        ...
        self.local_inference_dir = os.path.join(base_dir, "local_inference")
        self._create_dirs()

    def save_detection(self, detection: DetectionData) -> Dict[str, Any]:
        """保存检测数据 - 修改：只有有检测结果时才保存图像"""
        try:
            result = {"status": "success", "saved_files": []}

            # 如果是本地推理结果，使用专门的保存方法
            if detection.local_inference:
                return self.save_local_inference_result(detection)

            # 保存检测数据（无论是否有检测结果，都保存JSON用于记录）
            ...
            # 只在有检测结果时才保存图像
            if detection.has_detections() and detection.has_image():
    ...
    def save_local_inference_result(self, detection: DetectionData) -> Dict[str, Any]:
        """保存本地推理结果"""
        return detection.save_local_inference_result(self.local_inference_dir)

    def save_image_manually(self, detection: DetectionData) -> Dict[str, Any]:
        """手动保存图像 - 即使没有检测结果也保存"""
        try:
            result = {"status": "success", "saved_files": []}

            if detection.has_image():
                image_path = detection.save_image_manually(self.images_dir)
                if image_path:
                    result["saved_files"].append(image_path)
                    result["image_saved"] = True
                    result["has_detections"] = detection.has_detections()
                else:
                    result["image_saved"] = False
            else:
                result["image_saved"] = False
    ...
    def get_file_list(self, file_type: str = "all") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if file_type in ["all", "local_inference"]:
                search_paths.append(self.local_inference_dir)
...
# ====================== 本地推理工作线程 ======================
class LocalInferenceWorker(QThread):
    """本地推理工作线程"""

    # 定义信号
    progress_updated = pyqtSignal(int, int, str)  # 当前进度，总数量，当前文件
    inference_result = pyqtSignal(object, dict)  # DetectionData, 推理结果
    inference_finished = pyqtSignal(list, dict)  # 所有结果列表，统计信息
    error_occurred = pyqtSignal(str)

    def __init__(self, image_paths, model_path, save_dir, conf_threshold=0.25):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            if not YOLO_AVAILABLE:
                self.error_occurred.emit("ultralytics库未安装，无法进行本地推理.")
                return
            ...
            # 加载YOLO模型
            self.progress_updated.emit(0, len(self.image_paths), "正在加载YOLO模型...")
            try:
                self.model = YOLO(self.model_path)
            except Exception as e:
                self.error_occurred.emit(f"加载YOLO模型失败: {str(e)}")
                return
            ...
            for i, image_path in enumerate(self.image_paths):
                ...
                self.progress_updated.emit(i + 1, len(self.image_paths), f"正在处理: {filename}")
                ...
                try:
                    ...
                    if self.save_dir:
                        try:
                            ...
                            save_result = detection_data.save_local_inference_result(self.save_dir)
                        except Exception as e:
                    ...
                    # 发射推理结果信号
                    self.inference_result.emit(detection_data, save_result)
            ...
            stats['average_time_per_image'] = stats['total_time'] / len(self.image_paths) if self.image_paths else 0
            stats['detection_rate'] = stats['images_with_detections'] / len(self.image_paths) if self.image_paths else 0

            # 发射完成信号
            self.inference_finished.emit(results_list, stats)
...
class ServerThread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.statistics = {
            'total_frames': 0,
            'total_frames_with_detections': 0,  # 新增：有检测结果的帧数
            'total_detections': 0,
            ...
            def upload_result():
                try:
                    ...
                    if detection.has_detections():
                        self.statistics['total_frames_with_detections'] += 1
                        ...
                        if self.file_manager:
                            ...
                            self.log_signal.emit(f"检测到缺陷，已保存图像，检测数: {detection.get_detection_count()}", "info")
                    else:
                        # 没有检测结果，只记录JSON数据
                        save_result = {"status": "skipped", "message": "无缺陷检测，未保存图像"}
                        self.log_signal.emit(f"无缺陷检测，未保存图像", "info")
                    ...
                    self.detection_signal.emit({
                        ...
                        'has_detections': detection.has_detections()  # 新增字段
                    })

                    # 发送检测图像信号到GUI（无论是否有检测结果都显示）
                    ...
                    self.status_signal.emit({
                        ...
                        'total_frames_with_detections': self.statistics['total_frames_with_detections'],
                    ...
                    return jsonify({
                        ...
                        "image_saved": detection.has_detections()  # 告知客户端是否保存了图像
                    })
...
class MainWindow(QMainWindow):
    ...
    local_inference_progress = pyqtSignal(int, int, str)
    local_inference_result = pyqtSignal(object, dict)
    local_inference_finished = pyqtSignal(list, dict)
    local_inference_error = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        # 添加重置标志
        self.is_resetting_client = False

        # 本地推理相关
        self.local_inference_worker = None
        self.selected_local_images = []
        self.local_inference_results = []
        self.current_local_image_index = 0
        self.local_model_path = "best.pt"  # 默认模型路径

        # 检查模型是否存在
        self.model_exists = os.path.exists(self.local_model_path)

        # 初始化UI
        self.init_ui()

        # 设置窗口属性
        self.setWindowTitle("YOLO木材检测缺陷系统 - 主机端")
        ...
        # 在初始化后自动获取主机IP
        QTimer.singleShot(100, self.auto_get_local_ip)

    def auto_get_local_ip(self):
        """自动获取本机IP地址（在初始化后调用）"""
        try:
            # 创建一个UDP套接字来获取本机IP
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            # 这里不需要真正连接，只是为了获取本机IP
            s.connect(("8.8.8.8", 80))
            local_ip = s.getsockname()[0]
            s.close()

            # 更新主机IP输入框
            self.host_input.setText(local_ip)

            # 显示状态信息
            self.statusBar().showMessage(f"已自动获取本机IP地址: {local_ip}", 3000)

        except Exception as e:
            # 如果自动获取失败，使用默认IP
            self.host_input.setText("192.168.0.112")
            self.statusBar().showMessage(f"自动获取IP失败，使用默认IP: 192.168.0.112", 3000)
    ...
    def connect_signals(self):
        ...
        # 本地推理信号
        self.local_inference_progress.connect(self.update_local_inference_progress)
        self.local_inference_result.connect(self.handle_local_inference_result)
        self.local_inference_finished.connect(self.handle_local_inference_finished)
        self.local_inference_error.connect(self.handle_local_inference_error)
    ...
    def init_ui(self):
        ...
        self.create_local_inference_tab()  # 新增本地推理标签页
    ...
    def create_connection_tab(self):
        ...
        # 获取本机IP按钮
        self.get_local_ip_button = QPushButton("获取本机IP")
        self.get_local_ip_button.clicked.connect(self.get_local_ip)
        self.get_local_ip_button.setToolTip("点击自动获取本机IP地址")
        host_layout.addWidget(self.get_local_ip_button)
        ...
        # 树莓派IP - 修改：移除获取本机IP按钮
        raspberry_layout = QHBoxLayout()
        raspberry_layout.addWidget(QLabel("树莓派IP:"))
        self.raspberry_ip_input = QLineEdit("192.168.0.127")
        self.raspberry_ip_input.setPlaceholderText("请输入树莓派IP地址")
        raspberry_layout.addWidget(self.raspberry_ip_input)
        server_layout.addLayout(raspberry_layout)
        ...
        self.test_button.setToolTip("测试主机与树莓派的双向连接")
        ...
        self.frame_label = QLabel("接收总帧数: 0")
        status_layout.addWidget(self.frame_label)

        self.frame_with_detections_label = QLabel("有缺陷帧数: 0")  # 新增
        status_layout.addWidget(self.frame_with_detections_label)
    ...
    def create_file_management_tab(self):
        ...
        self.file_type_combo.addItems(["全部文件", "图像文件", "检测文件", "统计文件", "本地推理结果"])
        ...
        self.selected_file_info.setStyleSheet("""
            QTextEdit {
    ...
    def create_monitoring_tab(self):
        ...
        self.data_table.setColumnCount(7)  # 增加一列
        self.data_table.setHorizontalHeaderLabels(["ID", "时间", "客户端IP", "检测数", "图像尺寸", "状态", "有无缺陷"])
        ...
        """)
        # 设置初始文本
        self.stats_text.setText("服务器未运行，无统计信息")
    ...
    def create_local_inference_tab(self):
        """创建本地推理标签页"""
        ...
        self.model_path_input = QLineEdit(self.local_model_path)
        self.model_path_input.setReadOnly(True)
        model_path_layout.addWidget(self.model_path_input)

        self.browse_model_button = QPushButton("选择模型...")
        ...
        self.select_images_button = QPushButton("选择图像...")
        self.select_images_button.clicked.connect(self.select_local_images)
        image_button_layout.addWidget(self.select_images_button)

        self.clear_images_button = QPushButton("清空列表")
        self.clear_images_button.clicked.connect(self.clear_local_images)
        ...
        self.save_results_check = QCheckBox("保存推理结果")
        ...
        # 状态标签
        self.inference_status_label = QLabel("就绪")
        control_layout.addWidget(self.inference_status_label)

        # 按钮布局
        button_layout = QHBoxLayout()

        self.start_inference_button = QPushButton("开始推理")
        self.start_inference_button.clicked.connect(self.start_local_inference)
        self.start_inference_button.setEnabled(False)
        self.start_inference_button.setStyleSheet("background-color: #4CAF50; color: white; font-weight: bold;")
        button_layout.addWidget(self.start_inference_button)

        self.stop_inference_button = QPushButton("停止推理")
        self.stop_inference_button.clicked.connect(self.stop_local_inference)
        self.stop_inference_button.setEnabled(False)
        self.stop_inference_button.setStyleSheet("background-color: #f44336; color: white; font-weight: bold;")
        button_layout.addWidget(self.stop_inference_button)
        ...
        self.inference_stats_text.setText("未开始推理")
        stats_layout.addWidget(self.inference_stats_text)
        ...
        self.tab_widget.addTab(inference_tab, "本地推理")

    def create_settings_tab(self):
        """创建设置标签页 - 修改：增大系统信息框，占满整个页面的下半空余部分"""
        ...
        # 保存策略提示
        save_policy_label = QLabel("注意：服务器只会自动保存有缺陷检测的图像，但可以手动保存任何图像")
        save_policy_label.setStyleSheet("color: #ff9800; font-weight: bold;")
        save_layout.addWidget(save_policy_label)
        ...
        # 移除最大高度限制，让系统信息框可以扩展
        self.sys_info_text.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        # 设置样式，使其更清晰易读
        ...
        """)
        sys_layout.addWidget(self.sys_info_text, 1)  # 设置拉伸因子为1，使其可以扩展
        ...
        layout.addWidget(sys_group, 1)  # 设置拉伸因子为1，使系统信息组框可以扩展
    ...
    def update_model_status(self):
        ...
        if self.model_exists:
            self.model_status_label.setText(f"✅ 模型文件 '{self.local_model_path}' 存在")
            self.model_status_label.setStyleSheet("color: green; font-weight: bold;")
        else:
            self.model_status_label.setText(f"❌ 模型文件 '{self.local_model_path}' 不存在")
    ...
    def browse_model_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择YOLO模型文件",
            "",
            "YOLO模型文件 (*.pt *.pth);;所有文件 (*.*)"
        )

        if file_path:
            self.local_model_path = file_path
            self.model_path_input.setText(file_path)
            self.model_exists = os.path.exists(file_path)
            self.update_model_status()
    ...
    def select_local_images(self):
        ...
        if file_paths:
            self.selected_local_images = file_paths
            self.selected_images_list.clear()
            for path in file_paths:
                self.selected_images_list.addItem(os.path.basename(path))

            self.clear_images_button.setEnabled(True)
            self.start_inference_button.setEnabled(True and self.model_exists)

            self.inference_status_label.setText(f"已选择 {len(file_paths)} 张图像")
    ...
    def clear_local_images(self):
        ...
        self.clear_images_button.setEnabled(False)
        self.start_inference_button.setEnabled(False)
        self.inference_status_label.setText("就绪")

    def start_local_inference(self):
        """开始本地推理"""
        if not self.selected_local_images:
            QMessageBox.warning(self, "警告", "请先选择要推理的图像文件")
            return

        if not self.model_exists:
            QMessageBox.warning(self, "警告", f"模型文件 '{self.local_model_path}' 不存在")
            return

        if not YOLO_AVAILABLE:
            QMessageBox.warning(self, "警告", "ultralytics库未安装，无法进行本地推理。")
            return

        # 确认开始推理
        reply = QMessageBox.question(
            self, "开始本地推理",
            f"确定要对 {len(self.selected_local_images)} 张图像进行推理吗？",
        ...
        if self.save_results_check.isChecked():
            if self.file_manager.base_dir:
                save_dir = self.file_manager.local_inference_dir
            else:
                # 如果没有设置基础目录，使用当前目录下的local_inference文件夹
                save_dir = os.path.join(os.getcwd(), "local_inference")

        # 创建并启动推理工作线程
        self.local_inference_worker = LocalInferenceWorker(
            self.selected_local_images,
        ...
        # 连接信号
        self.local_inference_worker.progress_updated.connect(self.local_inference_progress)
        self.local_inference_worker.inference_result.connect(self.local_inference_result)
        self.local_inference_worker.inference_finished.connect(self.local_inference_finished)
        self.local_inference_worker.error_occurred.connect(self.local_inference_error)
        ...
        self.inference_status_label.setText("正在推理中...")

        # 启动线程
        self.local_inference_worker.start()

    def stop_local_inference(self):
        """停止本地推理"""
        if self.local_inference_worker and self.local_inference_worker.isRunning():
            reply = QMessageBox.question(
                self, "停止推理",
                "确定要停止推理吗？",
                QMessageBox.Yes | QMessageBox.No,
            ...
            if reply == QMessageBox.Yes:
                self.local_inference_worker.stop()
                self.inference_status_label.setText("推理已停止")

    def update_local_inference_progress(self, current, total, message):
        """更新推理进度"""
        if total > 0:
            progress = int((current / total) * 100)
            self.inference_progress.setValue(progress)
            self.inference_status_label.setText(f"{message} ({current}/{total})")

    def handle_local_inference_result(self, detection_data, save_result):
        """处理单个推理结果"""
        # 添加到结果列表
        self.local_inference_results.append(detection_data)

        # 更新当前显示的图像（如果是第一张或最后一张）
        if len(self.local_inference_results) == 1 or len(self.local_inference_results) == len(
                self.selected_local_images):
            self.current_detection_data = detection_data

            # 如果当前在检测画面标签页，更新显示
            if self.is_display_tab_active:
                # 在工作线程中处理图像
        ...
        self.statusBar().showMessage(
            f"本地推理: 已处理 {len(self.local_inference_results)}/{len(self.selected_local_images)} 张图像", 2000)

    def handle_local_inference_finished(self, results_list, stats):
        """处理推理完成"""
        # 更新UI状态
        self.start_inference_button.setEnabled(True)
        self.stop_inference_button.setEnabled(False)
        self.select_images_button.setEnabled(True)
        self.clear_images_button.setEnabled(len(self.selected_local_images) > 0)

        # 更新进度条和状态标签
        self.inference_progress.setValue(100)
        self.inference_status_label.setText("推理完成")
...
有缺陷图像数: {stats['images_with_detections']}
总检测数: {stats['total_detections']}
检测率: {stats['detection_rate'] * 100:.1f}%
总时间: {stats['total_time']:.2f} 秒
平均每张图像时间: {stats['average_time_per_image']:.2f} 秒

类别分布:
"""
        for class_name, count in stats['class_distribution'].items():
            stats_text += f"{class_name}: {count} 次\n"
        ...
        QMessageBox.information(self, "推理完成",
                                f"推理完成！\n\n"
                                f"已处理 {len(results_list)} 张图像\n"
                                f"检测到缺陷的图像: {stats['images_with_detections']} 张\n"
        ...
        # 清理工作线程
        if self.local_inference_worker:
            self.local_inference_worker = None

    def handle_local_inference_error(self, error_message):
        """处理推理错误"""
        ...
        self.inference_status_label.setText("推理出错")
    ...
    def get_local_ip(self):
        ...
        try:
            ...
            # 同时更新状态栏显示
            self.statusBar().showMessage(f"已获取本机IP地址: {local_ip}", 3000)

        except Exception as e:
            QMessageBox.warning(self, "获取IP失败", f"无法获取本机IP地址: {str(e)}")
    ...
    def stop_server(self):
        ...
        # 记录用户选择
        if reply == QMessageBox.Yes:
            self.user_choice_on_stop = "exit"
        else:
            self.user_choice_on_stop = "reset"
    ...
    def on_server_stopped(self):
        """服务器已停止信号处理"""
        # 根据用户选择执行相应操作
        if hasattr(self, 'user_choice_on_stop'):
            if self.user_choice_on_stop == "exit":
                # 退出客户端
                QTimer.singleShot(500, self.close_application)
            else:
                # 重置客户端
                QTimer.singleShot(500, self.reset_client)
        else:
            # 默认重置客户端
            QTimer.singleShot(500, self.reset_client)
    ...
    def close_application(self):
        ...
        if self.local_inference_worker and self.local_inference_worker.isRunning():
            self.local_inference_worker.stop()
            self.local_inference_worker.wait(1000)
    ...
    def reset_client(self):
        ...
        try:
            # 设置重置标志
            self.is_resetting_client = True

            # 清空图像显示
            self.image_label.clear()
            self.image_label.setText("等待接收图像...")
            self.image_info_label.setText("图像信息: 未接收")
            self.save_image_button.setEnabled(False)
            ...
            self.connection_label.setText("连接数: 0")
            self.frame_label.setText("接收总帧数: 0")
            self.frame_with_detections_label.setText("有缺陷帧数: 0")
            self.detection_label.setText("检测总数: 0")

            # 重置状态栏
            self.server_status_label.setText("服务器: 停止")
            self.image_status_label.setText("图像: 未接收")
            self.statusBar().showMessage("客户端已重置，就绪")
            ...
            # 显示重置完成消息
            QMessageBox.information(self, "客户端重置", "客户端已成功重置！\n\n现在可以重新配置并启动服务器。")

        except Exception as e:
            print(f"重置客户端时出错: {e}")
            QMessageBox.warning(self, "重置错误", f"重置客户端时出错: {str(e)}")
        finally:
            self.is_resetting_client = False
            self.is_stopping_server = False
    ...
    def test_connection(self):
        """测试连接 - 修改：同时测试主机与树莓派的双向连接"""
        host = self.host_input.text().strip()
        port = self.port_input.text().strip()
        raspberry_ip = self.raspberry_ip_input.text().strip()

        if not host or not port or not raspberry_ip:
            QMessageBox.warning(self, "警告", "请输入主机IP、端口和树莓派IP")
            return

        # 在工作线程中测试连接
        def test_connection_task(host, raspberry_ip):
            import subprocess
            results = []
            ...
            try:
                ...
                if result.returncode == 0:
                    results.append(f"✅ 主机IP [{host}] 连接测试: 成功")
                else:
                    results.append(f"❌ 主机IP [{host}] 连接测试: 失败")
                    overall_success = False

            except subprocess.TimeoutExpired:
                results.append(f"❌ 主机IP [{host}] 连接测试: 超时")
                overall_success = False
            except Exception as e:
                results.append(f"❌ 主机IP [{host}] 连接测试: 失败 - {str(e)}")
                overall_success = False

            try:
                # 测试树莓派IP连通性
                param = '-n' if os.name == 'nt' else '-c'
                result = subprocess.run(['ping', param, '1', raspberry_ip],
                                        capture_output=True, text=True, timeout=5)

                if result.returncode == 0:
                    results.append(f"✅ 树莓派IP [{raspberry_ip}] 连接测试: 成功")
                else:
                    results.append(f"❌ 树莓派IP [{raspberry_ip}] 连接测试: 失败")
                    overall_success = False

            except subprocess.TimeoutExpired:
                results.append(f"❌ 树莓派IP [{raspberry_ip}] 连接测试: 超时")
                overall_success = False
            except Exception as e:
                results.append(f"❌ 树莓派IP [{raspberry_ip}] 连接测试: 失败 - {str(e)}")
                overall_success = False

            # 测试双向网络连通性建议
            suggestions = []
            if overall_success:
                suggestions.append("🎉 双向连接测试全部成功！两端可以正常通信。")
            else:
                suggestions.append("⚠️  双向连接测试存在问题，请检查：")
                suggestions.append("1. 确保所有设备连接到同一网络")
                suggestions.append("2. 检查防火墙设置是否允许ping")
                suggestions.append("3. 验证IP地址是否正确")
                suggestions.append("4. 尝试重启网络设备")

            # 组合结果
            result_text = "双向连接测试结果：\n\n"
            result_text += "\n".join(results)
            result_text += "\n\n" + "\n".join(suggestions)

            return result_text, overall_success

        worker = Worker(test_connection_task, host, raspberry_ip)
    ...
    def show_test_result(self, message: str, success: bool):
        """显示测试结果"""
        if success:
            QMessageBox.information(self, "双向连接测试", message)
        else:
            QMessageBox.warning(self, "双向连接测试", message)
    ...
    def handle_status(self, status_data: dict):
        ...
        if status == 'starting':
            ...
            total_frames_with_detections = status_data.get('total_frames_with_detections', 0)
            ...
            self.frame_label.setText(f"接收总帧数: {total_frames}")
            self.frame_with_detections_label.setText(f"有缺陷帧数: {total_frames_with_detections}")
            self.detection_label.setText(f"检测总数: {total_detections}")

            # 更新状态栏
            self.statusBar().showMessage(
                f"服务器运行中 | 连接: {len(connected_clients)} | 总帧: {total_frames} | 有缺陷帧: {total_frames_with_detections}",
                1000)
    ...
    def handle_detection(self, detection_data: dict):
        ...
        has_detections = detection_data.get('has_detections', False)

        if has_detections:
            if save_result.get('status') == 'success':
                self.data_table.setItem(0, 5, QTableWidgetItem("已保存图像"))
                # 设置绿色背景
                item = QTableWidgetItem("已保存图像")
                item.setBackground(QColor(200, 255, 200))
                self.data_table.setItem(0, 5, item)
            else:
                item = QTableWidgetItem("保存失败")
                item.setBackground(QColor(255, 200, 200))
                self.data_table.setItem(0, 5, item)
        else:
            item = QTableWidgetItem("无缺陷，未保存")
            item.setBackground(QColor(240, 240, 240))
            self.data_table.setItem(0, 5, item)

        # 有无缺陷列
        has_detections_text = "有" if has_detections else "无"
        has_detections_item = QTableWidgetItem(has_detections_text)
        if has_detections:
            has_detections_item.setForeground(QColor(255, 0, 0))  # 红色
        else:
            has_detections_item.setForeground(QColor(0, 128, 0))  # 绿色
        self.data_table.setItem(0, 6, has_detections_item)
    ...
    def update_image_info(self, detection_data):
        """更新图像信息"""
        has_detections = detection_data.has_detections()
        ...
        info += f"状态: {'有缺陷' if has_detections else '无缺陷'}"
    ...
    def save_current_image(self):
        """保存当前图像 - 修改：即使没有检测到缺陷也保存，并添加Save_Manually后缀"""
        if not self.current_detection_data or not self.file_manager.base_dir:
            QMessageBox.warning(self, "警告", "没有可保存的图像或未设置保存路径")
            return

        # 询问用户确认
        has_detections = self.current_detection_data.has_detections()
        if has_detections:
            message = f"当前图像检测到 {len(self.current_detection_data.detections)} 个缺陷。\n是否保存当前图像？"
        else:
            message = "当前图像未检测到缺陷。\n是否仍然保存当前图像？"

        reply = QMessageBox.question(self, "保存图像", message,
        ...
        try:
            # 使用手动保存方法，即使没有检测结果也保存
            save_result = self.file_manager.save_image_manually(self.current_detection_data)

            if save_result.get("status") == "success" and save_result.get("image_saved"):
                filepath = save_result["saved_files"][0] if save_result["saved_files"] else "未知"
                if has_detections:
                    message = f"图像已保存到:\n{filepath}\n\n文件名包含'Save_Manually'后缀。"
                else:
                    message = f"图像已保存到:\n{filepath}\n\n文件名包含'Save_Manually'后缀。"
    ...
    def update_file_info(self):
        ...
        if row < len(self.current_files):
            ...
            # 改进文件信息显示格式，使用更丰富的HTML格式
            file_size_mb = file_info['size'] / (1024 * 1024)
            if file_size_mb >= 1:
                size_display = f"{file_size_mb:.2f} MB"
            else:
                size_display = f"{file_info['size'] / 1024:.2f} KB"

            file_type_display = ""
            if file_info['type'] == 'image':
                file_type_display = "📷 图像文件"
            elif file_info['type'] == 'detection':
                file_type_display = "📄 检测文件"
            else:
                file_type_display = f"📊 {file_info['type']}文件"

            # 修改：删除提示部分，每种信息单独一行
            info_text = f"""
<b>📋 文件信息</b>
<hr>
<b>📁 文件名:</b> {file_info['name']}<br>
<b>📍 路径:</b> {file_info['path']}<br>
<b>📄 类型:</b> {file_type_display}<br>
<b>📏 大小:</b> {file_info['size']:,} 字节 ({size_display})<br>
<b>🕒 创建时间:</b> {file_info['created'].strftime("%Y-%m-%d %H:%M:%S")}<br>
<b>🔄 修改时间:</b> {file_info['modified'].strftime("%Y-%m-%d %H:%M:%S")}
            """
            self.selected_file_info.setHtml(info_text.strip())
    ...
    def update_stats_display(self):
        ...
        if self.server_thread:
            # 计算统计数据
            total_frames = self.server_thread.statistics['total_frames']
            total_frames_with_detections = self.server_thread.statistics['total_frames_with_detections']
            total_detections = self.server_thread.statistics['total_detections']
            connected_clients = list(self.server_thread.statistics['connected_clients'])
            uptime = time.time() - self.server_thread.statistics['start_time']

            # 计算百分比
            detection_rate = (total_frames_with_detections / total_frames * 100) if total_frames > 0 else 0
...
<b>📊 服务器统计信息</b>
<hr>
<b>📈 总接收帧数:</b> {total_frames:,}
<b>✅ 有缺陷帧数:</b> {total_frames_with_detections:,} ({detection_rate:.1f}%)
<b>🔍 总检测数:</b> {total_detections:,}
<b>👥 连接客户端数:</b> {len(connected_clients)}
<b>⏱️  运行时间:</b> {uptime_str}
<b>📡 客户端列表:</b> {', '.join(connected_clients) if connected_clients else '无'}
<hr>
<i>💡 统计信息每分钟自动更新</i>
            """
    ...
    def update_system_info(self):
        """更新系统信息 - 修改：只保留操作系统、Python版本、处理器、可用内存，并让每一项独占一行"""
...
<br>
<b>💾 可用内存:</b><br>
{available_memory_gb:.2f} GB
                """
                return sys_info.strip()
            except Exception as e:
                return f"<b>❌ 获取系统信息失败:</b><br>错误信息: {str(e)}<br><br>请确保已安装必要的库:psutil"
    ...
    def set_sys_info_text(self, text: str):
        """设置系统信息文本"""
        self.sys_info_text.setHtml(text)
...
def main():
    ...
    app.setApplicationName("YOLO-木材缺陷检测系统 - 主机端")
